...
static bool cmd_is(std::string &word, const char *cmd);
...
static bool str_hash(const uint8_t *data, size_t len);
...
int main() {
  ...
  // a map of all client connections, keyed by id
    ...
    for (size_t i = 1; i < poll_args.size(); ++i) {
      if (poll_args[i].revents) {
        ...
        if (conn->state == STATE_END) {
          // client closed normaly, or something bad happend.
...
static bool try_fill_buffer(Conn *conn) {
  ...
  // Why is thre a loop? Please read the explanation of "pipelining".
...
static bool try_one_request(Conn *conn) {
  ...
  // note: need better handling for produciton code.
...
static bool str_hash(const uint8_t *data, size_t len) {
  // this is may be a 32-bit FNV hash function, but it is somewhat different.
...
static void msg(const char *msg) { printf("%s\n", msg); }
